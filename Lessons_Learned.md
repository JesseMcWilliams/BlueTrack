...
- The same no-pandoc/no-Python gap means `App Documents/Development_Host_Setup_Guide.docx` couldn't be updated when the CyberArk CP dependency was added 2026-09-01 (`BlueTrack.Api.csproj` now references `NetStandardPasswordSDK.dll` from a local CP install by absolute path — a real build prerequisite on any machine that needs to build `App/Api`). **Follow-up:** add a step to that guide for installing the CyberArk Credential Provider / Application Password SDK before building.
