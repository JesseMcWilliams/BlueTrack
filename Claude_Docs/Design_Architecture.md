# BlueTrack: Architecture

> **Stage: Design.** How the system works **now**, at the whole-system level. Each subsystem has its own `Design_*.md`; decisions are in `Design_Decision-Register.md`. Keep this current and under about 500 lines.
...
Deploy/          Install/Backup/Restore PowerShell scripts and modules -- see Deploy/README.md
Claude_Docs/     Design_ (one per subsystem, plus the Decision Register), Planning_, Testing_,
                 Reference_ (lessons learned) and Archive_ docs
User_Docs/       Short, linear, role-based walkthroughs (Admin_*/User_*)
Published_Docs/  .docx deliverables (Installation, Dev Host Setup), refreshed when a release is created
Reference/       Sample CyberArk Privileged Cloud/Self-Hosted export files (gitignored) for local dev/first load
```
...
- `User_Docs/Admin_DeploymentRunbook.md` — step-by-step environment stand-up/rebuild procedure
- `Claude_Docs/Design_Deployment-Methodology.md` — build/publish/IIS-deploy process
- `Claude_Docs/Design_Admin-Deployment-Management.md` — the Deployment admin page itself
