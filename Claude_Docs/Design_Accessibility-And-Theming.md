...
CSS custom-property **tokens** (`--color-bg`, `--color-text`, `--color-border`, `--color-link`, `--color-focus-ring`, plus status colors for success/error/warning) defined at `:root`, swapped by a `data-theme="Light" | "Dark" | "HighVisibility"` attribute set on `<html>` (**corrected 2026-09-09, documentation audit** — this previously read `"light" | "dark" | "high-contrast"`, which never matched the actual values this same document's own Data Model table below, and `App/Web/src/assets/themes.css`/`stores/theme.js`, have always used). `color-scheme: light dark` is set at `:root` so native browser chrome (scrollbars, form control defaults) follows the light/dark half of the system automatically; the High-Visibility theme isn't a browser "color scheme" in that sense, so it's handled purely through the custom-property layer.
