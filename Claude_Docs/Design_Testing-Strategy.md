...
- **`DevFakeAuth` is the test harness for authentication and permissions, not a new mechanism.** It already exists for exactly this purpose (Design_Authentication-Architecture.md's own framing: "not a separate authentication mechanism," a real Negotiate handshake with substituted group membership) — CI never needs a real domain, real Kerberos tickets, or real AD group SIDs to exercise `[Authorize(Policy = ...)]` checks.
...
**Corrected 2026-09-21 (documentation audit) — this Implementation Status had gone stale and been "corrected" by stacking a new paragraph below the old one, twice (2026-09-05, 2026-09-09), rather than replacing it — the same regrowth pattern already found and trimmed once in `Design_Decision-Register.md`'s own footer. Collapsed back to one accurate block instead of a layered history:**

- **Layer 1-3 (`App/Api.Tests`, xUnit):** `dotnet test` reports **441 passing** (grown from an original 57 through several dedicated coverage-expansion passes, most recently D-119/D-121/D-137/D-138, the rollback/segregation-of-duties/`db_backupstatus_reader`/audit-purge work, and D-143's KPI Summary report), verified by an actual run against `BlueTrackTest`, not estimated. Contract tests authenticate via `TestAuthHandler`, a test-only `AuthenticationHandler` that stamps the same `bluetrack:provider_type=DevFakeAuth` marker claim OIDC/SAML use, so `WebApplicationFactory` exercises the real authorization pipeline without a real Negotiate handshake (which `TestServer` can't perform at all — discovered directly building this, see `BlueTrackWebApplicationFactory`'s own comment).
- **Layer 1 (`App/Web`, Vitest):** **20 files, 126 tests** (up from 2 files/18 tests as of 2026-09-09), verified by an actual run. Grown from just the `rights`/`theme` Pinia store tests to real component-level coverage across most admin/list/report pages, following the pattern `RiskScoreBands.test.js` first established (D-120) once this project had no prior Vitest component coverage to extend.
- **Layer 4 (`App/E2E`, Playwright):** **8 spec files, 57 `test()` definitions** (up from 7 files/49 as of 2026-09-09) — `targets-and-access-groups.spec.js` (D-121) is the new file. This count is a **static count of `test()` calls in the spec files, not a live run** — per this document's own established stance below, a full Playwright run isn't re-verified on every documentation pass given the known proxy-flakiness cost; re-run it directly if you need actual pass/fail numbers, not just how many tests exist. Role-switching goes through a dev-only endpoint, `App/Api/Controllers/DevTestAuthController.cs` (`GET /api/auth/dev/test-signin`, 404 outside Development) — a real Negotiate-authenticated browser can only ever be the one Windows account it runs as, so DevFakeAuth's normal per-username lookup can't switch roles per test on its own.
- **Test fixtures:** `Database/Test/01_BlueTrack_Test_DevFakeAuthMatrixSeed.sql` seeds synthetic `TestUser.Viewer` / `.Analyst` / `.Approver` / `.Admin` identities (never a real person's account) with a test-fixture-only Viewer/Analyst/Approver permission matrix; `Admin` reuses the real bootstrap role from `09_BlueTrack_WebSeed.sql` (renamed from `07_BlueTrack_WebInterface_Seed.sql` in the 2026-09-05 script restructure — this reference was never updated until now).
...
2. ~~**Coverage starting point**~~ — done. Permission enforcement (layers 1-4) and Account Progress editing (locking, validation, audit trail together, Risk Exceptions) are both covered — see `account-progress-and-risk-exceptions.spec.js` and the broader xUnit/Vitest counts above. D-91 confirmed this comprehensive against this document's own stated scope.
