...
| PermissionName | text, unique | e.g. `ViewDashboard`, `EditAccountProgress`, `ApproveExceptions`, `ManageIdentityProviders`, `ManageGroupRoleMapping`, `CuratePlatformMapping`, `ConfirmReconciliation`, `ReloadRights`, `ManageRolesAndPermissions`, `CurateApplicationMapping`, `ManageSecretsStore`, `ManageFieldMetadata`, `ViewAuditLog`, `ManageApplicationConfiguration`, `ViewDeploymentInfo`, `ManageTargets`, `ManageAccessGroups`, `ViewRiskReport`, `ManageRiskScoreBands` — **updated 2026-08-27 (D-61)** to cover every admin page in the finalized page inventory (`Design_Application-Structure.md`); the last six were missing on review. **`ViewDeploymentInfo` added 2026-09-05 (documentation audit) for D-98**, which had updated the actual database/seed scripts but never made it back into this catalog listing. **`ManageTargets`/`ManageAccessGroups`/`ViewRiskReport` added 2026-09-09 (documentation audit) for D-119 (Risk Scoring)**, same class of gap as `ViewDeploymentInfo` above — the database/seed scripts and `Permissions.cs` had them; this catalog listing didn't. **`ManageRiskScoreBands` added 2026-09-09 for D-120 (Risk Score Bands, `Design_Risk-Scoring.md` Phase F)**, gating the new admin page that maps named bands onto the computed risk score. |
...
**Resolved 2026-09-04 (D-94): confirmed as literal default rows, not just illustrative.** The user confirmed directly — seed Viewer/Analyst/Approver as real default roles (`10_BlueTrack_DefaultRoleSeed.sql`), plus a new **Auditor** role scoped to `ViewAuditLog` only, deliberately kept separate from Viewer/Analyst/Approver (none of which include `ViewAuditLog`) so a person can be given audit-log visibility without also getting dashboard/account-progress read access. `Admin` is unchanged (the existing bootstrap role, `09_BlueTrack_WebSeed.sql`) and remains the only other role with `ViewAuditLog` by default.
