...
| PerformedByUserKey | FK to app_user | **Resolved 2026-08-27 (D-59):** `app_user`, not `dim_user` — see `Design_Authentication-Architecture.md`. Who did it — the authenticated identity, not a raw provider claim |
...
| Reason | text, nullable | **New 2026-08-27** (D-51): a structured, required-when-applicable justification for an edit — e.g. why a Blueprint stage was regressed (`Design_Data-Editing-Behavior.md`). Null for edits that don't require justification. |
...
**Retention/purge: actually built 2026-09-21, roughly a month after `web.audit_purge_log` was created alongside it.** `usp_PurgeAuditLog` (`Database/39_BlueTrack_AuditLogPurgeProcedure.sql`) was, until this pass, only ever mentioned in that table's own schema comment — a real, confirmed gap found while writing `User_Docs/Admin_OperationsGuide.md`'s day-2 content, not something this document itself had been tracking as open. Deletes `audit_field_change` rows before their parent `audit_event` rows (FK dependency) older than `RetentionDays` days, records the run in `audit_purge_log`, and no-ops with a logged `'Skipped'` status if `RetentionDays` is still unset — matching this document's own original design exactly, just built a month later than the schema that anticipated it. `Database/40_BlueTrack_ScheduleAuditLogPurgeJob.sql` schedules it nightly at 3:00 AM, following the same "never through `App/Migrator`, msdb-only" convention as `14`/`38` — see `User_Docs/Admin_OperationsGuide.md` for the operational side (checking whether it ran, what `'Skipped'` means, re-running manually).
...
Resolved: audit scope defaults to writes/logons/approvals with reads as an off-by-default option (Q-26/D-35); break-glass alerting resolved separately as D-24 in `Design_Authentication-Architecture.md`; app-user identity resolved as D-59 in `Design_Authentication-Architecture.md`.
