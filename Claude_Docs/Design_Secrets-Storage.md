...
**Windows DPAPI** (`WindowsDpapiProtector`) — built 2026-09-04. Uses `DataProtectionScope.LocalMachine`. Verified with a real encrypt-then-decrypt round trip on this host. **Correction, 2026-09-05 (documentation audit): this claimed "has no caller yet," which is now stale** — `ILocalSecretProtector.Protect`/`Unprotect` is actively called from two places: `IdentityProviderRepository` (OIDC's `PlaintextSecret`, D-84) and `SecretsStoreRepository` (the write-only `PlaintextCredential` used by CyberArk Conjur, Azure Key Vault's ServicePrincipal auth, and AWS Secrets Manager's AccessKey auth, D-84). One or more distinct credentials can be, and today are, protected this way simultaneously — `Protect`/`Unprotect` takes the plaintext/ciphertext value itself as its argument, with no shared state between calls, so there's no limit on how many independent secrets it protects at once.

### DPAPI protection scope — configurable, not fixed (D-106, 2026-09-05)

`LocalMachine` scope was chosen because it decrypts correctly regardless of whether the app pool account's Windows user profile is loaded, which `CurrentUser` scope depends on — but `LocalMachine` is also the *weaker* boundary: any process able to run code as an account with sufficient local privilege on this machine (not just the specific app pool identity) can decrypt `LocalMachine`-scoped ciphertext. Confirmed 2026-09-05: DPAPI's protection should have a configurable option to scope it to the *specific* account running the application pool instead — a local OS account or a domain service account, either is expected — for a materially tighter security boundary: only that one account (or someone able to log on as it) can decrypt, not anyone with sufficient privilege on the box.

**Proposed:** a new `ProtectionScope` field in `web.secrets_store.BackendSettings` for the `WindowsDpapi` row (`"LocalMachine"` default, or `"CurrentUser"`), read by `WindowsDpapiProtector` instead of the scope being hardcoded — matching the same per-backend `BackendSettings` JSON pattern every other Secrets Store backend already uses (D-95). Switching to `CurrentUser` needs one real IIS prerequisite called out explicitly, since it's exactly the failure mode `LocalMachine` was originally chosen to avoid: the application pool's **"Load User Profile" advanced setting must be `true`**, or the CLR has no loaded user profile to derive the `CurrentUser` DPAPI master key from, and every `Protect`/`Unprotect` call throws a `CryptographicException`. This is not a new setting on `WindowsDpapiProtector` to build around — it's an IIS-side operational prerequisite that must be true *before* switching `ProtectionScope` to `CurrentUser`, worth a plain warning in whatever admin UI exposes this setting.

**This does not change D-65's accepted disaster-recovery risk — if anything, it narrows the recovery bar further, deliberately.** `CurrentUser` scope is still bound to the same originating machine (DPAPI's fundamental limitation, unchanged), *plus* now also bound to the specific account — a backup restored to different hardware, or even the same hardware under a different app pool identity, still can't decrypt either way. `LocalMachine` remains the pragmatic default for exactly this reason (one fewer thing that has to match on restore); `CurrentUser` is offered as an opt-in hardening option for an environment that values the tighter access-control boundary over that small amount of extra restore fragility, not a fix for the underlying machine-binding risk itself.
...
**Documentation audit correction, 2026-09-09: a sixth real `IVaultSecretProvider` implementation, `WindowsDpapiVaultSecretsProvider`, was never added here.** Built under D-118 and registered in `App/Api/Program.cs` (`AddScoped<IVaultSecretProvider, WindowsDpapiVaultSecretsProvider>()`) — distinct from the `WindowsDpapiProtector`/`ILocalSecretProtector` pair documented above (D-79's split), and only documented so far in `Design_Credentials-Management.md`'s own D-118 section, which this doc's Implementation Status never cross-references. See that document for the actual behavior/verification detail.
...
- **`AwsSecretsManagerSecretsProvider`** — `AWSSDK.SecretsManager`, supporting IAM Role (default, the SDK's own credential chain) or an explicit Access Key auth method.
- **`CyberArkConjurSecretsProvider`** — a hand-rolled REST client against Conjur's publicly documented Authn API (no official .NET SDK exists). **Does not resolve or reverse D-33's deferral** — Conjur's real contract is still unconfirmed with the platform team; this is scaffolding, not a verified integration.

All three reinterpret `SecretQuery`'s Safe/Folder/Object shape for their own concept of a secret reference (each provider's own doc comment explains its mapping). Where a backend needs a credential to authenticate to its own remote service, a new convention applies: DPAPI-protected inline in `BackendSettings` under a shared `ProtectedCredential` key, set via a new write-only `PlaintextCredential` field (`SecretsStoreController`/`SecretsStoreRepository`) and redacted (`"***"`) on every read — a backend can't depend on "the active Secrets Storage backend" to bootstrap its own credential, since it might *be* that backend. `SecretsStoreConfiguration.vue` now exposes a Credential input column for this.

**Verified end to end** against real (rejecting) endpoints: Azure Key Vault against an unreachable placeholder vault URI, correctly classified `VaultConnectivity` (after a real bug was found and fixed — the Azure SDK wraps connection failures in `AggregateException`, not the documented `RequestFailedException`, which escaped as an unhandled 500 until caught by this test); AWS Secrets Manager against the real, live AWS endpoint with a fake access key, correctly classified `AccessDenied` (after fixing a categorization gap — AWS returns HTTP 400 `UnrecognizedClientException` for a bad key, not 401/403); CyberArk Conjur against an unreachable placeholder host, correctly classified `VaultConnectivity`. CyberArk CCP was regression-checked as still the active, working backend throughout.

Nothing in the app yet actually *consumes* a secret from any Secrets Storage (`IVaultSecretProvider`) backend for a real feature — every backend built so far has been proven only via the admin test action, not a real caller. (OIDC's client secret, D-85, is a real `ILocalSecretProtector` consumer, but deliberately bypasses the Secrets Storage/`IVaultSecretProvider` abstraction entirely — see `Design_Authentication-Architecture.md` for why a startup-time credential can't go through the active-backend vault lookup.)

## Open Questions

None remaining as of 2026-08-27. Q-08 is fully resolved: architecture (D-16), backend/build order (D-32, D-36), authentication (D-38), call/response shape (D-39), the `Folder` qualifier (D-40), error handling/caching (D-48), and availability/naming/app-level resilience (D-49). Still true as of the 2026-09-05 updates above (D-106's configurable DPAPI protection scope, and correcting the "no caller yet" claim) — documentation corrections and a hardening option, not new open questions.
