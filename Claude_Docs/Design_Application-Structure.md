...
- Login (default-provider redirect with a small link to other enabled providers, including break-glass — see `Design_Authentication-Architecture.md` D-41)
...
- Deployment — environment/version info, health checks, SQL Server backup status, gated by `ViewDeploymentInfo` (D-98, `Design_Admin-Deployment-Management.md`)
...
- Credentials & LDAP — vault-backend credential management plus LDAP config, gated by `ManageCredentials` (`Design_Credentials-Management.md`). **Updated 2026-09-16 (D-137)**: LDAP config is now a real list, one row per AD domain/forest (previously a singleton) — needed once the AD Account Discovery feature had to query potentially more than one domain.
- Target Match Review / Import Mapping Profiles — the Risk Scoring import admin pages, gated by `ManageTargets` (D-119, `Design_Risk-Scoring.md`; the import-mapping mechanism Import Mapping Profiles manages is designed in the companion `Design_Risk-Scoring-Import.md`, split out 2026-09-21). Targets/Access Groups themselves **moved out** of this hub to their own top-level nav entries (D-121, see Analyst-Facing above) — these two stay here, unmoved.
- Risk Score Bands — admin-configurable named bands (e.g. Low/Medium/High/Critical) over the computed `EffectiveRiskScore`, surfaced on the Account Progress list and Risk Score report; NOT the same thing as `dim_risk_level`. Gated by `ManageRiskScoreBands` (D-120, `Design_Risk-Scoring.md` Phase F)
...
- **Unresolved Entitlement Members** — Safe entitlements granted to a member this app can't resolve to a known user/group (D-145/D-147); no permission gate, read-only.
- **Risk Score** — sortable list of computed/override/effective account risk scores with a per-account contributor drill-down, gated by `ViewRiskReport` (D-119, `Design_Risk-Scoring.md`).
- **Discovered Accounts** (added 2026-09-16, D-137) — real AD accounts not yet onboarded into CyberArk, found nightly by matching AD group membership against the Access Group inventory (D-119) and risk-scored the same way, with a per-account drill-down showing which Access Groups it matched. Gated by `ViewDiscoveredAccounts`. **Updated 2026-09-16 (D-138)**: Accept/Dismiss actions (gated separately by `ManageDiscoveredAccounts`) move a candidate into real Blueprint tracking or resolve it as a false positive — Accept creates a real `dbo.fact_account` row immediately visible in the Account Progress list, reusing `dbo.dim_source_system`'s previously-unused `'DISCOVERY'` source. Still never talks to CyberArk directly — actual Vault onboarding remains a separate, human/CyberArk-side action.
- **Risk Exception SoD** (added 2026-09-16) — every historical case where the same person both approved a Risk Exception and later linked it to an account, built entirely from the existing audit trail. Gated by `ViewRiskExceptionSodReport`. Shown regardless of whether the segregation-of-duties enforcement toggle (Global Application Configuration) is or ever was on — a detective control, not tied to the enforcement setting. See `User_Docs/User_Guide.md`.
- **KPI Summary** (added 2026-09-22, D-143) — four nested progress ratios: In Scope vs. All Accounts, Onboarded vs. In Scope, Managed vs. Onboarded, and Compliant vs. Managed. "In scope" reuses the existing Risk Accepted / Excluded status (D-19/D-59) rather than a new exception type — an account leaves scope only through that same, already-enforced accepted-exception mechanism. Onboarded/Managed are `dim_blueprint_stage` thresholds (StageOrder >= 3 / >= 4); Compliant requires both the final stage and status Complete specifically. No permission gate, read-only, matching Overdue/At-Risk and Stage/Status Summary above. The same five underlying counts also back a new "Key Progress Indicators" section on the Dashboard (below), including a pie chart of five mutually-exclusive funnel buckets summing to the total account count.

More report types can be added the same way later; this isn't meant to be exhaustive. **Documentation audit correction, 2026-09-09**: this list only had the original three as of 2026-08-27 — the last two were added since (D-145/D-147, D-119) but never made it back into this list. **Documentation audit correction, 2026-09-21**: same gap recurred — Risk Exception SoD (added the same day as Discovered Accounts) was missing from this list too, found while doing a broader documentation-completeness pass.

## Data Model

### app_config

**New 2026-08-27 (D-60).** A gap found on review: `audit_config` (`Design_Audit-Logging.md`) only holds audit-specific settings (`RetentionDays`, `LogReadEvents`), but the Global Application Configuration page also needs to hold settings that have nothing to do with auditing. Kept as a separate table rather than folding into `audit_config`, so audit-specific and general settings stay cleanly separated as more global settings get added later.
...
**Added 2026-09-10, list-page UX overhaul (D-124-D-129, D-136)** — conventions added since the original D-42 pass, applied consistently across every list page (Account Progress, Targets, Access Groups, and the rest sharing the same patterns):

- **Server-side pagination** (D-124 Phase 3): `page`/`pageSize` query params, `X-Total-Count`/`X-Filtered-Count` response headers (the JSON body itself stays a bare array, non-breaking) — a shared `Pager.vue` component and `pageSize` Pinia store (persisted server-side per user) back a page selector that appears both above and below the table (D-126, the bottom one is a copy of the top one, not a different component).
- **Add/Edit and Bulk Import are full pages, not modals** (D-124) — "New"/"Bulk" render as buttons placed below the page description (D-126, previously plain links at the top), navigating to their own route rather than opening in place.
- **Click the row's name to edit it** (D-125) — the name cell is the edit link, not a separate "Edit" action column; Cancel (and Save) on the edit page both return to the list, never to a separate summary/detail page in between.
- **Every Delete button confirms first** (D-128), via a single shared `ConfirmDialog.vue`/`useConfirmDialog.js` singleton (promise-based, mounted once in `App.vue`) rather than the browser's native `confirm()` or a per-page dialog. Access Group/Target deletes use a structured `{ title, details, warning }` message — Name/Scope/Address/Source lines in a visually indented block, separated from the title/warning around it (D-129/D-131) — every other Delete button passes a plain string, both forms supported by the same composable.
- **Table styling** (D-136, `themes.css`, applies to every `<table>` in the app at once): full grid lines on every cell (not just horizontal row separators), a `tbody tr:hover` background highlight (`--color-row-hover`, a new token per theme, contrast-verified the same way every other theme token is), and a sticky `<thead>` that stays visible while a long list scrolls. Zebra striping was considered and deliberately not added, same reasoning as the original 2026-09-06 table-styling pass: an alternating-row tint against the hand-designed High-Visibility AAA palette (`Design_Accessibility-And-Theming.md`) needs the same contrast rigor as every other color in that file.
- **Multi-select for bulk actions across selected rows** — mentioned by the user as a planned next feature (2026-09-10) but not yet designed or built; noted here as a placeholder so it isn't lost, not as something already in place.
...
**Resolved 2026-08-27 (D-41, cross-referenced from `Design_Authentication-Architecture.md`):** displayed provider/module names are admin-configurable (`DisplayName` on `identity_provider_config`, already part of the Authentication Architecture data model) — this is reaffirmed here as a UI-structure requirement, not a new field.
...
**Dashboard.vue — built 2026-09-05 (D-99), added here since this document's Page Inventory names it above but the original Implementation Status entry (2026-09-01) predates it.** Three summary cards built entirely from existing endpoints, no new backend rollups: accounts-by-stage totals (`GET /api/reports/stage-status-summary`), an overdue/at-risk accounts count with a link into that worklist (`GET /api/reports/overdue-at-risk`), and risk exceptions needing attention — an overdue-review count any authenticated user sees, plus an active-exceptions-awaiting-approval count shown only to users holding `ApproveExceptions` (checked client-side so most roles never attempt a call that would 403). See `Design_Decision-Register.md` D-99 and the Login page's own D-100 (also built the same day, closing out every remaining literal-placeholder page named in this document's Page Inventory). **Updated 2026-09-22 (D-143)**: the Accounts by Stage card's hand-rolled inline SVG pie chart was never mentioned here despite existing in code for some time (no charting library dependency — see D-143's own repository note for the shared slice-building logic). A fourth card, "Key Progress Indicators," was added the same day: the four KPI Summary ratios above as percentages, plus a second pie chart of the same five mutually-exclusive funnel buckets (Out of Scope, In Scope-Not Onboarded, Onboarded-Not Managed, Managed-Not Compliant, Compliant), reusing the first pie's arc-drawing code rather than duplicating it.

**List-page UX overhaul — done (2026-09-10, D-119-D-136, `feature/list-page-ux-overhaul`).** Pagination, Add/Edit-as-page, click-name-to-edit, delete confirmation, and full-grid/hover/sticky-header table styling are all now live across Account Progress/Targets/Access Groups — see the Lists & Grids section above for the conventions themselves and `Design_Decision-Register.md` D-119 through D-136 for exhaustive per-decision detail (schema, live-verification results, test counts). Not duplicated here to avoid the two documents drifting out of sync with each other.

## Open Questions

None remaining as of 2026-08-27 — still true as of the 2026-09-05 updates above (Playwright coverage, Dashboard.vue, the `app_config` schema refresh); none of them raised a new open question. Still true as of the 2026-09-10 list-page UX overhaul above.
