...
**Retention/purge: actually built 2026-09-21, roughly a month after `web.audit_purge_log` was created alongside it.** `usp_PurgeAuditLog` (`Database/39_BlueTrack_AuditLogPurgeProcedure.sql`) was, until this pass, only ever mentioned in that table's own schema comment — a real, confirmed gap found while writing `Design_Operations_Guide.md`'s day-2 content, not something this document itself had been tracking as open. Deletes `audit_field_change` rows before their parent `audit_event` rows (FK dependency) older than `RetentionDays` days, records the run in `audit_purge_log`, and no-ops with a logged `'Skipped'` status if `RetentionDays` is still unset — matching this document's own original design exactly, just built a month later than the schema that anticipated it. `Database/40_BlueTrack_ScheduleAuditLogPurgeJob.sql` schedules it nightly at 3:00 AM, following the same "never through `App/Migrator`, msdb-only" convention as `14`/`38` — see `Design_Operations_Guide.md` for the operational side (checking whether it ran, what `'Skipped'` means, re-running manually).
