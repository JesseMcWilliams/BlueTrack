...
**Split out to `Design_Risk_Scoring_Import.md` on 2026-09-21** (tracked in `Audit_Findings_Tracker.md` — this document had grown to 375 lines, the largest in this folder by more than 2x). That companion document covers the CSV bulk-upload pattern (Targets/Access Groups/direct Account→Target links), the ETL load procedures, and the configurable field-mapping layer (`web.import_mapping_profile`/`import_mapping_field`) all four ETL-fed imports share. See it for the full detail — nothing about the data model above changed as part of that split.
...
- **Import-mapping open questions** (ETL feed file shapes, interactive mapping UX, mapping a risk score directly on import) moved to `Design_Risk_Scoring_Import.md`'s own Open Questions section.
...
*Split 2026-09-21: this document had grown to 375 lines, more than 2x any other document in this folder (`Audit_Findings_Tracker.md`'s own long-flagged concern). The import-mapping/ETL half moved to `Design_Risk_Scoring_Import.md` — data model, matching, algorithm, admin pages, and the full Implementation Status build history stay here.*
