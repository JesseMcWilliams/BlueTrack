...
Phases B (bulk CSV/import mapping/target matching), C (`PendingSafeDerived` auto-linking), D (scoring computation), and E (Account Progress integration + Risk Score report) are still in progress — see the approved implementation plan for the full phase breakdown.
