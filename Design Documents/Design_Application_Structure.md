...
- Target Match Review / Import Mapping Profiles — the Risk Scoring import admin pages, gated by `ManageTargets` (D-119, `Design_Risk_Scoring.md`; the import-mapping mechanism Import Mapping Profiles manages is designed in the companion `Design_Risk_Scoring_Import.md`, split out 2026-09-21). Targets/Access Groups themselves **moved out** of this hub to their own top-level nav entries (D-121, see Analyst-Facing above) — these two stay here, unmoved.
...
- **Table styling** (D-136, `themes.css`, applies to every `<table>` in the app at once): full grid lines on every cell (not just horizontal row separators), a `tbody tr:hover` background highlight (`--color-row-hover`, a new token per theme, contrast-verified the same way every other theme token is), and a sticky `<thead>` that stays visible while a long list scrolls. Zebra striping was considered and deliberately not added, same reasoning as the original 2026-09-06 table-styling pass: an alternating-row tint against the hand-designed High-Visibility AAA palette (`Design_Accessibility_And_Theming.md`) needs the same contrast rigor as every other color in that file.
