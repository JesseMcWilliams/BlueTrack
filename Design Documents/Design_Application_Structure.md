...
- **D-42 (multi-layer filter/sort) — partially resolved 2026-09-04.** Account Progress List now has the real thing: simultaneous filters (Stage/Status/Risk Level/Owner-contains, stacked with AND) plus true multi-column sort (click a header to sort by it alone, shift-click another to add it as a secondary key, with numbered arrow badges showing priority) — `AccountProgressRepository.GetSummaryListAsync`/`AccountProgressController.ParseSort`, sort columns validated against a fixed whitelist rather than interpolating the requested field name directly (the requested sort field comes straight from the query string, so this is the SQL-injection guard, not just tidiness — verified with an actual injection attempt in the `sort` parameter, safely ignored). **Risk Exceptions and Audit Log Viewer still only have their original single-value filters** — the same pattern just hasn't been extended to them yet.
