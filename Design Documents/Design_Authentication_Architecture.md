...
- **Operational/IdP-side setup guidance added 2026-09-23**: the exact redirect URI/ACS URL/SP metadata endpoint values a real IdP needs from BlueTrack, the OIDC secret-missing silent-failure gap, and how these map onto Okta/Entra ID/Ping/Authentik's own terminology — added to `Design_Admin_Deployment_Management.md` once this exact documentation gap was found and asked about directly. Not repeated here, to keep this document's own scope on architecture and decisions rather than operational how-to.
