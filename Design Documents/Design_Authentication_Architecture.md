...
- **Not built**: an actual frontend login-provider-selector screen (D-41's "default provider first, small link for the rest") — designing that meaningfully is hard to do blind with zero real providers to click through, so it's left as a follow-up once real IdP values exist to test against. SAML's Single Logout round trip to the IdP is also not built (`Saml2Controller.Logout` only signs out of this app's own session) — same reasoning.

## Login Flow

1. User requests a protected page.
2. If exactly one provider is enabled, redirect directly to it. **Resolved 2026-08-27 (D-41):** if more than one provider is enabled, the login screen defaults straight to the configured default provider (the user's last-used one per D-02, or an admin-configured system default before any preference is recorded) rather than presenting a full choice screen up front — a small link exposes the other enabled providers, including break-glass, for the user who needs a different one. Break-glass users are expected to recognize its label without additional in-app explanation.
