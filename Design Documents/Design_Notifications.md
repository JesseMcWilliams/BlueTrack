...
**One real MailKit finding from live testing**: MailKit's `AuthenticateAsync(username, password)` auto-negotiates whichever SASL mechanism the server advertises (PLAIN/LOGIN/NTLM/...) -- the real Windows-authenticated relay needed no special-casing beyond a plain username/password credential to authenticate via NTLM. Separately, that relay's real TLS certificate passed hostname/chain validation but had no reachable CRL/OCSP endpoint from this network; `SmtpClient.CheckCertificateRevocation = false` (the user's explicit choice over treating the relay's certificate/CA as the thing to fix) skips only that check, hostname/chain validation stays fully enforced.
