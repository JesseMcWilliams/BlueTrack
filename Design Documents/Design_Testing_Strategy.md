...
- **Layer 1-3 (`App/Api.Tests`, xUnit):** one project, `Unit/`/`Integration/`/`Contract/` folders — `dotnet test` reports 338 passing (46 files carry `[Fact]`/`[Theory]` attributes; the runtime count is higher than the attribute count since a `[Theory]` with `MemberData` is one attribute but many actual cases, e.g. `AdminControllersPermissionTests`) as of 2026-09-09 (D-119's Risk Scoring work), grown well past the original 57 via several dedicated coverage-expansion PRs, #6-10, culminating in D-91's "confirmed comprehensive" audit against this document's own stated scope, and further coverage added with every feature since. Contract tests authenticate via `TestAuthHandler`, a test-only `AuthenticationHandler` that stamps the same `bluetrack:provider_type=DevFakeAuth` marker claim OIDC/SAML use, so `WebApplicationFactory` exercises the real authorization pipeline without a real Negotiate handshake (which `TestServer` can't perform at all — discovered directly building this, see `BlueTrackWebApplicationFactory`'s own comment).
- **Layer 1 (`App/Web`, Vitest):** 2 files now — `src/stores/rights.test.js` and `src/stores/theme.test.js` (added with D-93's theming work) — 18 tests total against the real `rights`/`theme` Pinia stores.
- **Layer 4 (`App/E2E`, Playwright):** 7 spec files (`account-progress-and-risk-exceptions`, `admin-pages`, `dashboard-and-login`, `list-pages`, `permission-boundaries`, `reports-pages`, `theme`), 49 tests total as of 2026-09-09 (D-119 added Risk Score report and risk-score-override coverage to `reports-pages.spec.js`/`account-progress-and-risk-exceptions.spec.js`), covering every Admin/Reports/list page plus Account Progress editing (locking, Risk Exceptions, risk score overrides), Dashboard, Login, and theming — not just `permission-boundaries.spec.js` alone. Role-switching goes through a dev-only endpoint, `App/Api/Controllers/DevTestAuthController.cs` (`GET /api/auth/dev/test-signin`, 404 outside Development) — a real Negotiate-authenticated browser can only ever be the one Windows account it runs as, so DevFakeAuth's normal per-username lookup can't switch roles per test on its own.
