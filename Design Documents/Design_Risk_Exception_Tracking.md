...
**Still deferred, by the design's own words:** application-scoped exceptions still don't propagate to every account under that application's Safes — "the exact mechanics of that resolution (a view vs. a batch update) are an implementation detail for later, not decided here" (see `fact_account_progress.ExceptionKey`'s own row above). D-77 only wires the account-scoped path; application-scoped linking isn't exposed anywhere in the Account Progress edit form yet.
